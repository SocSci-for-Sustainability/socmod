...
knitr::opts_chunk$set(
  ...
  out.width = "50%",
  dpi=300,fig.width=2,fig.height=1.5,fig.align="center"
)
# knitr::opts_chunk$set(dpi=300,fig.width=2,fig.height=1.5,fig.align="center")
...
make_example_abm <- function(...) {
  ...
  make_example_agents <- function() {
    ...
    agents <- purrr::map(
      1:n_agents, \(a_idx) { 
        Agent$new(id = a_idx,
                  behavior=agent_behaviors[a_idx], 
  ...
  return (AgentBasedModel$new(make_model_parameters(graph = socnet), agents = agents, ...))
}
...
```{r, fig.align="center", fig.width =2}
library(ggplot2)
abm <- make_example_abm()

trial <- run_trial(abm, 50)

plot_adoption(trial)
#ggplot(result$get_observations(), aes(x=t, y=)) +
 # geom_line() +
  #xlab("Time step") +
  #ylab("# Adaptive") +
  #theme_classic()
...
`socmod` provides social network tools to create and analyze custom instances of `igraph::Graph`. See examples below. Create and pass these to the agent-based
models constructor to create ABMs with specified network structures, e.g., 
```{r}
...
```{r, fig.height = 1.5, fig.width = 2, out.width="70%", dpi=400}
...
```{r, fig.height = 2.25, fig.width = 3, out.width="65%", dpi=300}
