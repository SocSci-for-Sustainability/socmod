...
# Load socmod from local source if available (during dev), otherwise use installed version
...
observations$Behavior <- factor(observations$Behavior, levels = c("Legacy", "Adaptive"))
...
# Run trials using run_trials_grid
trials <- run_trials_grid(
  f_A = c(2, 4, 6, 8),
  strategy = names(strategies),
  partner_selection = strategies |> purrr::map("partner_selection"),
  interaction = strategies |> purrr::map("interaction"),
  reps = 25,
  model_generator = function(f_A) {
    function() {
      abm <- make_example_learning_abm()
