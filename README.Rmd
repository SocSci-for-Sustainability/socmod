...
The goal of `socmod` is to simplify development of models of social behavior for beginners and experts alike. `socmod` focuses on agent-based models for now, but the framework is flexible to be extended to other iterative or non-iterative models of behavior diffusion, opinion dynamics, and other social behaviors. 
...
We can succinctly write a single pipeline to do the following three analyses outlined below, 1, 2A, and 2B.

1. Initialize, run, and visualize prevalence time series for a single trial
2. Define a computational experiment that runs a number of trials to be analyzed in two ways: (i) success rates and mean times to fixation over model parameter values
and (ii) average prevalence time series over model parameter values.
...
4B. summarized across input variables (second example), or...
4C. analyze & visualize the outcomes (third example)
...
```{r, fig.height = 1.15, fig.width = 1.15, dpi=500}
...
```{r fig.height = 1.35, fig.width = 1.55, dpi=500}
...
```{r fig.height = 1.25, fig.width = 1.95, dpi=500}
...
```{r fig.width = 2.65, fig.height = 1.95} 
